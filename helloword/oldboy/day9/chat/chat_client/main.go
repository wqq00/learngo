...
import (
	"net"
	"fmt"
	"learngo/learngo/helloword/oldboy/day9/chat/proto"
	"encoding/json"
	"encoding/binary"
	"errors"
)

func readPackage(conn net.Conn) (msg proto.Messge, err error){
	var buf [8192]byte
...
func register(conn net.Conn) (err error){
	var msg proto.Messge
	msg.Cmd = proto.UserRegister

	var loginCmdRes proto.LoginCmdRes

	err = json.Unmarshal([]byte(msg.Data), &loginCmdRes)
	if err != nil {
		return
	}
